...
const AboutCards = () => {
  ...
  return (
    <div className="h-[75vh] w-full pt-28 flex gap-4" ref={testRef}>
      {cardsData.map((item, index) => {
        return (
          <div
            key={index}
            className={`h-[50vh] w-[22vw] relative bg-[#24022C] p-4 ${
              index === 2
                ? "bottom-[16vh]"
                : index === 1 || index === 3
                ? "bottom-[8vh]"
      ...
      <div className="h-[400px] w-[400px] rounded-full bg-[#CF2379] blur-[10vw] absolute -bottom-52 right-52"></div>
    </div>
  );
};
